...
@section('page') {{ trans('forum::general.manage') }} @endsection
@section ('content')
    <div class="d-flex flex-row justify-content-between mb-2">
        <h2 class="flex-grow-1">{{ trans('forum::general.manage') }}</h2>

        @can ('createCategories')
            <button type="button" class="btn btn-primary rounded-pill" data-open-modal="create-category">
    ...
    <div id="manage-categories">
        ...
        <div class="text-end py-3">
            <button type="button" class="btn btn-primary rounded-pill px-5" :disabled="state.isSavingDisabled" @click="onSave">
        ...
        <draggable
            ...
            <template #item="{element}">
                <li class="list-group-item" :data-id="element.id">
                    <a class="btn btn-sm btn-danger rounded-pill float-end ml-2" :href="element.route + '#modal=delete-category'">{{ trans('forum::general.delete') }}</a>
                    <a class="btn btn-sm btn-link rounded-pill float-end ml-2" :href="element.route + '#modal=edit-category'">{{ trans('forum::general.edit') }}</a>
